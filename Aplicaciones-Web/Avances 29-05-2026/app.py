...
def patient_dashboard():
    if 'user_id' not in session or session['user_type'] != 'paciente':
        return redirect(url_for('login'))
    db = get_db()
    paciente = db.execute('SELECT * FROM pacientes WHERE id = ?', (session['user_id'],)).fetchone()
    lecturas = db.execute('SELECT * FROM lecturas WHERE paciente_id = ? ORDER BY dia DESC', (session['user_id'],)).fetchall()
    
    # Calcular el próximo día
    ultima_lectura = db.execute('SELECT MAX(dia) as max_dia FROM lecturas WHERE paciente_id = ?', (session['user_id'],)).fetchone()
    proximo_dia = (ultima_lectura['max_dia'] or 0) + 1
    
    return render_template('patient_dashboard.html', lecturas=lecturas, paciente=paciente, proximo_dia=proximo_dia)
...
def add_reading():
    ...
    if glucosa_ayunas:
        try:
            glucosa_ayunas = float(glucosa_ayunas)
            if glucosa_ayunas <= 15:
                flash('❌ ADVERTENCIA: Glucosa en ayunas ≤ 15 mg/dL es biológicamente imposible. Verificar con su doctor.')
                return redirect(url_for('patient_dashboard'))
        except ValueError:
            flash('Glucosa en ayunas debe ser un número válido.')
            return redirect(url_for('patient_dashboard'))

    if glucosa_2h:
        try:
            glucosa_2h = float(glucosa_2h)
            if glucosa_2h <= 40:
                flash('❌ ADVERTENCIA: Glucosa 2h ≤ 40 mg/dL es biológicamente imposible. Verificar con su doctor.')
                return redirect(url_for('patient_dashboard'))
        except ValueError:
            flash('Glucosa 2h debe ser un número válido.')
            return redirect(url_for('patient_dashboard'))
    ...
    if a1c:
        try:
            a1c = float(a1c)
            if a1c <= 3.5:
                flash('❌ ADVERTENCIA: A1C ≤ 3.5% es biológicamente imposible. Verificar con su doctor.')
                return redirect(url_for('patient_dashboard'))
        except ValueError:
            flash('A1C debe ser un número válido.')
            return redirect(url_for('patient_dashboard'))

    # Convertir otros valores
    try:
        ldl = float(ldl) if ldl else None
        hdl = float(hdl) if hdl else None
        trigliceridos = float(trigliceridos) if trigliceridos else None
        colesterol_total = float(colesterol_total) if colesterol_total else None
    except ValueError:
        flash('Todos los valores deben ser números válidos.')
        return redirect(url_for('patient_dashboard'))
