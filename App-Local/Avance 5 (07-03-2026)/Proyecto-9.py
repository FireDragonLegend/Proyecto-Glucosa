    ...
    def evaluate_diabetes_risk(self):
        ...
        if age is None:
            age = simpledialog.askinteger(
                ...
                "Nota: Para niños (8-12 años), se evalúa prediabetes infantil",
                minvalue=8,
                maxvalue=120
            )
            if age is None:
        ...
        ldl = None
        while ldl is None:
            ldl_str = simpledialog.askstring(
                "Colesterol LDL (Malo)",
            ...
            try:
                ldl = float(ldl_str)
                if ldl < 0 or ldl > 500:
                    messagebox.showwarning("Valor inválido", "Por favor ingrese un valor entre 0 y 500.")
                    ldl = None
            except ValueError:
        ...
        hdl = None
        while hdl is None:
            hdl_str = simpledialog.askstring(
            ...
            try:
                hdl = float(hdl_str)
                if hdl < 0 or hdl > 150:
                    messagebox.showwarning("Valor inválido", "Por favor ingrese un valor entre 0 y 150.")
                    hdl = None
            except ValueError:
                messagebox.showwarning("Error", "Ingrese un número válido.")
                hdl = None
        ...
        triglycerides = None
        while triglycerides is None:
            tri_str = simpledialog.askstring(
                "Triglicéridos",
            ...
            if tri_str is None:
                return
            try:
                triglycerides = float(tri_str)
                if triglycerides < 0 or triglycerides > 1000:
                    messagebox.showwarning("Valor inválido", "Por favor ingrese un valor entre 0 y 1000.")
                    triglycerides = None
            except ValueError:
                messagebox.showwarning("Error", "Ingrese un número válido.")
                triglycerides = None
        ...
        bmi = None
        while bmi is None:
            bmi_str = simpledialog.askstring(
            ...
            try:
                bmi = float(bmi_str)
                if bmi < 10 or bmi > 60:
                    messagebox.showwarning("Valor inválido", "Por favor ingrese un valor entre 10 y 60.")
                    bmi = None
            except ValueError:
                messagebox.showwarning("Error", "Ingrese un número válido.")
                bmi = None
        ...
        systolic = None
        while systolic is None:
            sys_str = simpledialog.askstring(
            ...
            try:
                ...
                if systolic < 80 or systolic > 250:
                    messagebox.showwarning("Valor inválido", "Por favor ingrese un valor entre 80 y 250.")
                    systolic = None
            except ValueError:
                messagebox.showwarning("Error", "Ingrese un número válido.")
                systolic = None
        ...
        diastolic = None
        while diastolic is None:
            dia_str = simpledialog.askstring(
            ...
            try:
                ...
                if diastolic < 50 or diastolic > 150:
                    messagebox.showwarning("Valor inválido", "Por favor ingrese un valor entre 50 y 150.")
                    diastolic = None
            except ValueError:
                messagebox.showwarning("Error", "Ingrese un número válido.")
                diastolic = None
        ...
        if has_diabetes_family:
            # Preguntas adicionales para refinar el riesgo familiar
            family_count = simpledialog.askinteger(
                "Antecedentes Familiares - Detalles",
                "¿Cuántos familiares de primer grado tienen diabetes?\n\n"
                "Ejemplos: padres, hermanos, hijos\n\n"
                "Nota: Más familiares = mayor riesgo",
                minvalue=1,
                maxvalue=10
            )
            if family_count is None:
                family_count = 1  # Valor por defecto

            family_details['family_count'] = family_count

            # Preguntar por tipos de parentesco
            relationships = []
            if messagebox.askyesno("Parentesco", "¿Tiene padre(s) con diabetes?"):
                relationships.append("padre")
            if messagebox.askyesno("Parentesco", "¿Tiene madre con diabetes?"):
                relationships.append("madre")
            if messagebox.askyesno("Parentesco", "¿Tiene hermano(s) con diabetes?"):
                relationships.append("hermano")
            if messagebox.askyesno("Parentesco", "¿Tiene hijo(s) con diabetes?"):
                relationships.append("hijo")

            family_details['relationship_types'] = relationships

            # Preguntar por edad de aparición promedio
            onset_ages = []
            for rel in relationships:
                age_onset = simpledialog.askinteger(
                    f"Edad de Aparición - {rel.capitalize()}",
                    f"¿A qué edad aproximadamente desarrolló diabetes su {rel}?\n\n"
                    "Si no sabe exactamente, estime lo mejor posible.\n"
                    "Esto ayuda a calcular mejor su riesgo personal.",
                    minvalue=10,
                    maxvalue=90
                )
            ...
            # Calcular factor de riesgo familiar refinado
            base_risk = 1.0
            if family_count >= 2:
                base_risk += 0.5  # Más familiares = mayor riesgo
            if "padre" in relationships or "madre" in relationships:
                base_risk += 0.3  # Padres = riesgo alto
            if "hermano" in relationships:
                base_risk += 0.2  # Hermanos = riesgo moderado
            if onset_ages and sum(onset_ages)/len(onset_ages) < 50:
                base_risk += 0.2  # Aparición temprana = mayor riesgo genético

            family_risk_factor = min(3.0, max(1.0, base_risk))
            family_details['risk_factor'] = family_risk_factor
        
        exercise_weekly = None
        while exercise_weekly is None:
            ex_str = simpledialog.askstring(
            ...
            if ex_str is None:
                return
            try:
                exercise_weekly = float(ex_str)
                if exercise_weekly < 0 or exercise_weekly > 1000:
                    messagebox.showwarning("Valor inválido", "Ingrese un valor entre 0 y 1000 minutos.")
                    exercise_weekly = None
            except ValueError:
                messagebox.showwarning("Error", "Ingrese un número válido.")
                exercise_weekly = None
        
        diet_quality = simpledialog.askinteger(
            "Calidad de la Dieta",
            "Califique su adherencia a una dieta saludable:\n\n"
            "1 = Pobre (muchos ultraprocesados, azúcares, grasas)\n"
            "2 = Regular (algo desordenada, pocos vegetales)\n"
            "3 = Buena (equilibrada con algunos excesos)\n"
            "4 = Muy Buena (mucha fruta, verdura, fibra, proteínas magras)\n"
            "5 = Excelente (muy controlada, posiblemente con nutricionista)",
            minvalue=1,
            maxvalue=5
        )
        ...
        care_score = 2.0
        care_score -= min(1.2, exercise_weekly / 200.0)  # Más ejercicio = mejor
        care_score -= (diet_quality - 1) * 0.25  # Mejor dieta = mejor
        care_score += family_history * 0.3  # Antecedentes = peor
        care_score = max(0.0, min(2.0, care_score))
        
        # PASO 7: Preparar características para el modelo avanzado (15 features)
        features = [
            age,                    # 0: Edad
            avg_fast,              # 1: Glucosa ayuno promedio
            avg_post,              # 2: Glucosa post-prandial promedio
            avg_a1c,               # 3: A1C promedio
            bmi,                   # 4: IMC
            ldl,                   # 5: LDL
            hdl,                   # 6: HDL
            triglycerides,         # 7: Triglicéridos
            total_chol,            # 8: Colesterol total
            systolic,              # 9: Presión sistólica
            diastolic,             # 10: Presión diastólica
            care_score,            # 11: Score de cuidados
            family_history,        # 12: Antecedentes familiares
            exercise_weekly,       # 13: Minutos ejercicio semanal
            diet_quality           # 14: Calidad de dieta (1-5)
        ]
        
        # PASO 8: Evaluación con modelo avanzado
        model = self.get_diabetes_model()
        if model is None:
            messagebox.showerror("Error IA", "No se pudo inicializar el modelo IA avanzado. El entrenamiento puede estar en curso.")
            return

        # Usar la nueva función de evaluación integral
        prob, stage, risk_factors, recommendations = assess_diabetes_risk_comprehensive(features, model)
        
        # PASO 9: Calcular timeframe con etapa y factor familiar
        ...
        # PASO 10: Evaluar síntomas por etapa y edad
        symptoms = possible_symptoms_by_probability(prob, stage=stage, age=age)
        
        # PASO 11: Evaluar perfil lipídico avanzado
        ...
        # Actualizar resumen en panel
        self.lbl_summary.config(text=report[:800] + "..." if len(report) > 800 else report)
        
        # Mostrar reporte completo
        messagebox.showinfo("Evaluación Avanzada de Riesgo", report)
