...
class GlucoseMonitorApp:
    def __init__(self, root):
        ...
        self.model_status = "No cargado"  # estado del modelo
    ...
    def _background_load_model(self):
        ...
        try:
            ...
            print("Iniciando carga del modelo IA en background...")
    ...
    def _train_model_from_db_thread(self):
        """Inicia entrenamiento del modelo desde BD en un hilo separado."""
        # Mostrar diálogo de confirmación
        if not messagebox.askyesno(
            "Entrenar modelo IA",
            "¿Desea entrenar la IA con los datos de la base de datos?\n\n"
            "Esto puede tomar un tiempo. El entrenamiento se hará en segundo plano."
        ):
            return
        
        # Iniciar el entrenamiento en un hilo para no bloquear la interfaz
        train_thread = threading.Thread(target=self._background_train_model, daemon=True)
        train_thread.start()

    def _background_train_model(self):
        """Método ejecutado en segundo plano para entrenar el modelo desde BD."""
        try:
            print("\n🧠 Iniciando entrenamiento del modelo en background...")
            result = self.train_model_from_database()
            if result:
                print("✓ Entrenamiento completado exitosamente")
                self.model_status = "Modelo entrenado desde BD"
        except Exception as e:
            print(f"✗ Error en entrenamiento background: {e}")
            self.model_status = f"Error en entrenamiento: {str(e)}"
    ...
    def load_or_train_model(self):
        ...
        if os.path.exists(model_path):
            ...
            print("Archivo de modelo no encontrado, entrenando modelo optimizado...")

        # Entrenar modelo optimizado (rápido)
        print("Entrenando modelo de IA optimizado (versión rápida)...")
        try:
            from diabetes_model import generate_advanced_dataset, AdvancedDiabetesModel

            print("Generando dataset optimizado...")
            X, y = generate_advanced_dataset(300)  # Dataset optimizado para velocidad
            print(f"Dataset generado: {len(X)} muestras")

            print("Entrenando modelo de IA optimizado...")
            model = AdvancedDiabetesModel(n_features=15)
            model.fit(X, y, epochs=50, batch_size=64, validation_split=0.2, patience=5)  # Entrenamiento rápido
            ...
            with open(model_path, 'wb') as f:
                pickle.dump(model, f)
            print("✓ Modelo optimizado entrenado y guardado exitosamente.")
            return model
        except Exception as e:
            ...
            messagebox.showerror("Error de IA",
                ...
                f"Error: {str(e)}\n\n"
                "El programa continuará sin predicciones avanzadas.")
            return None
    ...
    def train_model_from_database(self):
        """
        Entrena el modelo de IA usando datos de la base de datos.
        Recupera todos los registros y los prepara para entrenar.
        """
        try:
            # Recuperar todos los datos de la BD
            self.cursor.execute("""
                SELECT edad_registrada, ayuno, pos2h, a1c, bmi, ldl, hdl, triglycerides, 
                       total_cholesterol, systolic, diastolic, care_score, family_risk, 
                       exercise_weekly, diet_quality
                FROM lecturas
                WHERE edad_registrada IS NOT NULL 
                  AND ayuno IS NOT NULL 
                  AND pos2h IS NOT NULL 
                  AND a1c IS NOT NULL
            """)
            
            registros = self.cursor.fetchall()
            
            if not registros or len(registros) < 10:
                messagebox.showwarning(
                    "Datos insuficientes",
                    f"Se encontraron {len(registros) if registros else 0} registros.\n"
                    "Se necesitan al menos 10 registros con datos completos para entrenar.\n"
                    "Siga registrando mediciones para mejorar el modelo."
                )
                return False
            
            print(f"\n📚 Entrenando modelo desde DB con {len(registros)} registros...")
            
            # Preparar características (X) y etiquetas (y) desde los datos
            import random
            random.seed(42)
            ...
            for reg in registros:
                edad, ayuno, pos2h, a1c, bmi, ldl, hdl, trig, total_chol, systolic, diastolic, care_score, family_risk, exercise_weekly, diet_quality = reg
                
                # Llenar valores None con valores por defecto
                edad = edad if edad is not None else 45
                bmi = bmi if bmi is not None else 25
                systolic = systolic if systolic is not None else 120
                diastolic = diastolic if diastolic is not None else 80
                care_score = care_score if care_score is not None else 3.0
                family_risk = family_risk if family_risk is not None else 0
                exercise_weekly = exercise_weekly if exercise_weekly is not None else 150
                diet_quality = diet_quality if diet_quality is not None else 3
                
                # Las 15 características en el orden esperado por el modelo
                features = [
                    edad, ayuno, pos2h, a1c, bmi,
                    ldl, hdl, trig, total_chol,
                ...
                # Generar etiqueta basada en la severidad (simplificado)
                # Mayor riesgo si tiene múltiples factores de riesgo
                high_risk_count = sum([
                    ayuno >= 110, 
                    pos2h >= 160, 
                    a1c >= 6.0,
                ...
                # Etiquetar como riesgo si hay 3 o más factores
                label = 1 if high_risk_count >= 3 else 0
                y.append(label)
            
            print(f"Características preparadas: {len(X)} muestras")
            print(f"Distribución de etiquetas: {sum(y)} positivas, {len(y)-sum(y)} negativas")
            
            # Entrenar el modelo con los datos de BD
            if self.diabetes_model is None:
                from diabetes_model import AdvancedDiabetesModel
                self.diabetes_model = AdvancedDiabetesModel(n_features=15)
            
            print("Iniciando entrenamiento del modelo...")
            self.diabetes_model.fit(X, y, epochs=100, batch_size=4, validation_split=0.2, patience=10)
            
            # Guardar el modelo entrenado
            import os
            model_path = "diabetes_model.pkl"
            with open(model_path, 'wb') as f:
                pickle.dump(self.diabetes_model, f)
            
            print(f"✓ Modelo reentrenado y guardado exitosamente")
            messagebox.showinfo(
                "✓ Entrenamiento Exitoso",
                f"Modelo IA reentrenado exitosamente con {len(X)} registros de la BD.\n\n"
                f"Etiquetas: {sum(y)} casos de riesgo, {len(y)-sum(y)} casos sin riesgo\n\n"
                "El modelo ahora podrá hacer predicciones más precisas basadas en tus datos."
            )
            return True
            
        except Exception as e:
            print(f"✗ Error al entrenar desde BD: {e}")
            import traceback
            traceback.print_exc()
            messagebox.showerror(
                "Error de Entrenamiento",
                f"No se pudo entrenar el modelo desde la BD.\n"
                f"Error: {str(e)}"
            )
            return False
    ...
    def _build_ui(self):
        ...
        banner = ttk.Label(self.root, text="📊 Monitor de Glucosa", style="Header.TLabel")
        ...
        ttk.Label(frm_part, text="Fecha de Nacimiento (YYYY-MM-DD):").grid(row=6, column=0, sticky="w")
        self.new_fecha_nac = tk.StringVar()
        ttk.Entry(frm_part, textvariable=self.new_fecha_nac, width=12).grid(row=6, column=1, sticky="w")

        ttk.Label(frm_part, text="PDF de Exámenes:").grid(row=7, column=0, sticky="w")
        btn_pdf = ttk.Button(frm_part, text="Seleccionar PDF", command=self.seleccionar_pdf)
        btn_pdf.grid(row=7, column=1, sticky="w")

        btn_reg = ttk.Button(frm_part, text="Registrar participante", command=lambda: self.registrar_participante_db(self.new_nombre.get(), self.new_edad.get(), self.new_fecha.get(), self.new_lugar.get(), self.new_fecha_nac.get()), style="Primary.TButton")
        btn_reg.grid(row=8, column=0, columnspan=2, pady=6, sticky="we")
        ...
        ttk.Label(frm_inputs, text="LDL (Malo) (mg/dL):").grid(row=4, column=0, sticky="w", pady=4)
        self.ldl_var = tk.StringVar()
        ttk.Entry(frm_inputs, textvariable=self.ldl_var, width=12).grid(row=4, column=1, padx=6, pady=4, sticky="w")
        ttk.Label(frm_inputs, text="(Menos de 100 mg/dL óptimo)", font=("Segoe UI", 8), foreground="#6366f1").grid(row=4, column=2, sticky="w", padx=2)

        ttk.Label(frm_inputs, text="HDL (Bueno) (mg/dL):").grid(row=5, column=0, sticky="w", pady=4)
        self.hdl_var = tk.StringVar()
        ttk.Entry(frm_inputs, textvariable=self.hdl_var, width=12).grid(row=5, column=1, padx=6, pady=4, sticky="w")
        ttk.Label(frm_inputs, text="(60 mg/dL o más óptimo)", font=("Segoe UI", 8), foreground="#6366f1").grid(row=5, column=2, sticky="w", padx=2)

        ttk.Label(frm_inputs, text="Triglicéridos (mg/dL):").grid(row=6, column=0, sticky="w", pady=4)
        self.trig_var = tk.StringVar()
        ttk.Entry(frm_inputs, textvariable=self.trig_var, width=12).grid(row=6, column=1, padx=6, pady=4, sticky="w")
        ttk.Label(frm_inputs, text="(Menos de 150 mg/dL óptimo)", font=("Segoe UI", 8), foreground="#6366f1").grid(row=6, column=2, sticky="w", padx=2)

        ttk.Label(frm_inputs, text="Colesterol Total (mg/dL):").grid(row=7, column=0, sticky="w", pady=4)
        self.total_chol_var = tk.StringVar()
        ttk.Entry(frm_inputs, textvariable=self.total_chol_var, width=12).grid(row=7, column=1, padx=6, pady=4, sticky="w")
        ttk.Label(frm_inputs, text="(Menos de 200 mg/dL óptimo)", font=("Segoe UI", 8), foreground="#6366f1").grid(row=7, column=2, sticky="w", padx=2)

        btn_frame = ttk.Frame(frm_inputs)
        btn_frame.grid(row=8, column=0, columnspan=3, pady=(8, 0), sticky="we")
        ...
        self.lbl_status = ttk.Label(frm_inputs, text="", font=("Segoe UI", 8), foreground="#333")
        self.lbl_status.grid(row=9, column=0, columnspan=3, sticky="w", pady=(6, 0))

        util_frame = ttk.Frame(frm_inputs)
        util_frame.grid(row=10, column=0, columnspan=3, pady=6, sticky="we")
        ...
        btn_train_ai = ttk.Button(frm_expert, text="🧠 Entrenar IA desde BD", command=self._train_model_from_db_thread)
        btn_train_ai.pack(fill="x", pady=2)
        
        # Indicador de estado del modelo IA
        self.model_status_label = ttk.Label(frm_expert, text="Estado IA: Inicializando...", 
    ...
    def _update_model_status(self):
        """Actualiza el indicador de estado del modelo IA."""
    ...
    def crear_tablas_db(self):
        ...
        # Asegurar que la tabla existe antes de intentar eliminar registros
        self.cursor.execute("""
        ...
        # Asegurar columnas nuevas (fecha_nacimiento, pdf_examenes) si la tabla ya existía
        try:
            self.cursor.execute("ALTER TABLE participantes ADD COLUMN fecha_nacimiento TEXT")
        except sqlite3.OperationalError:
            pass
        try:
            self.cursor.execute("ALTER TABLE participantes ADD COLUMN pdf_examenes BLOB")
        except sqlite3.OperationalError:
            pass

        # Nota: no borramos participantes al iniciar para que los datos persistan entre sesiones
        ...
        # Agregar columnas para características adicionales de entrenamiento de IA
        try:
            self.cursor.execute("ALTER TABLE lecturas ADD COLUMN edad_registrada INTEGER")
        except sqlite3.OperationalError:
            pass
        try:
            self.cursor.execute("ALTER TABLE lecturas ADD COLUMN bmi REAL")
        except sqlite3.OperationalError:
            pass
        try:
            self.cursor.execute("ALTER TABLE lecturas ADD COLUMN systolic REAL")
        except sqlite3.OperationalError:
            pass
        try:
            self.cursor.execute("ALTER TABLE lecturas ADD COLUMN diastolic REAL")
        except sqlite3.OperationalError:
            pass
        try:
            self.cursor.execute("ALTER TABLE lecturas ADD COLUMN care_score REAL")
        except sqlite3.OperationalError:
            pass
        try:
            self.cursor.execute("ALTER TABLE lecturas ADD COLUMN family_risk REAL")
        except sqlite3.OperationalError:
            pass
        try:
            self.cursor.execute("ALTER TABLE lecturas ADD COLUMN exercise_weekly REAL")
        except sqlite3.OperationalError:
            pass
        try:
            self.cursor.execute("ALTER TABLE lecturas ADD COLUMN diet_quality INTEGER")
        except sqlite3.OperationalError:
            pass
        
        # Tabla para notas de profesionales
        self.cursor.execute("""
    ...
    def seleccionar_pdf(self):
        """Selecciona un archivo PDF para almacenar en BD."""
        file_path = filedialog.askopenfilename(filetypes=[("PDF files", "*.pdf")])
        if file_path:
            with open(file_path, 'rb') as f:
                self.pdf_data = f.read()
            messagebox.showinfo("PDF", "PDF seleccionado correctamente.")
    ...
    def cargar_lecturas_desde_db(self, participante_id):
        ...
        if participante_id is None:
            print("[DEBUG] cargar_lecturas_desde_db: participante_id es None")
            return
        print(f"[DEBUG] cargar_lecturas_desde_db: cargando lecturas para participante_id={participante_id}")
        self.cursor.execute("""
        ...
        """, (participante_id,))
        filas = self.cursor.fetchall()
        print(f"[DEBUG] cargar_lecturas_desde_db: filas encontradas={len(filas)}")
        for f in filas:
            entry = {
                'day': f[0],
                'date': f[1],
        ...
        print(f"[DEBUG] cargar_lecturas_desde_db: self.readings ahora={self.readings}")
    ...
    def registrar_participante_db(self, nombre, edad, fecha, lugar, fecha_nac):
        """Inserta un participante en BD y recarga combobox"""
        print(f"Intentando registrar: nombre='{nombre}', edad='{edad}', fecha='{fecha}', lugar='{lugar}', fecha_nac='{fecha_nac}'")  # Debug
        
        if not nombre.strip() or len(nombre.strip()) < 10:
            messagebox.showerror("Error", "El nombre debe tener al menos 10 letras.")
            return
        
        if not fecha_nac.strip():
            messagebox.showerror("Error", "La fecha de nacimiento es obligatoria.")
            return

        # Validar formato de fecha de nacimiento (admite varios separadores y formatos simples)
        fecha_nac_str = fecha_nac.strip().replace('.', '/').replace('-', '/')
        parsed_date = None
        for fmt in ("%Y/%m/%d", "%d/%m/%Y", "%m/%d/%Y"):
            try:
                parsed_date = datetime.strptime(fecha_nac_str, fmt)
                break
            except Exception:
                continue
        if parsed_date is None:
            messagebox.showerror(
                "Error",
                "Fecha de nacimiento inválida. Use formatos como YYYY-MM-DD, DD/MM/YYYY o MM/DD/YYYY."
            )
            return
        # Guardar en formato estándar (YYYY-MM-DD)
        fecha_nac = parsed_date.strftime("%Y-%m-%d")
        ...
        try:
            self.cursor.execute(
                "INSERT INTO participantes (nombre, edad, fecha, lugar, fecha_nacimiento, pdf_examenes) VALUES (?, ?, ?, ?, ?, ?)",
                (nombre.strip(), edad_val, fecha.strip(), lugar.strip(), fecha_nac.strip(), self.pdf_data)
            )
            self.conn.commit()
            new_id = self.cursor.lastrowid
            print(f"Participante registrado exitosamente en BD (id={new_id}).")  # Debug
            self.pdf_data = None  # reset

            # Actualizar lista y seleccionar el nuevo participante inmediatamente
            self.cargar_participantes_desde_db()
            self.current_participant_id = new_id
            self.participante_var.set(nombre.strip())
            self._refresh_participante_combobox(select_id=new_id)

            # Cargar lecturas en caso de que existan y actualizar la vista
            self.cargar_lecturas_desde_db(new_id)
            ...
            print(f"Error al registrar en BD: {e}")  # Debug
            messagebox.showerror("Error de BD", f"No se pudo registrar el participante: {e}")

    def guardar_lectura_en_db(self, participante_id, entry):
        """Inserta o actualiza (upsert) una lectura del participante con opción de colesterol y características de IA"""
        if participante_id is None:
            return
        # Campos de colesterol
        ...
        # Campos de características para reentrenamiento de IA
        edad_registrada = entry.get('edad_registrada', None)
        bmi = entry.get('bmi', None)
        systolic = entry.get('systolic', None)
        diastolic = entry.get('diastolic', None)
        care_score = entry.get('care_score', None)
        family_risk = entry.get('family_risk', None)
        exercise_weekly = entry.get('exercise_weekly', None)
        diet_quality = entry.get('diet_quality', None)
        
        self.cursor.execute("""
            INSERT INTO lecturas (participante_id, dia, fecha, ayuno, pos2h, a1c, ldl, hdl, triglycerides, total_cholesterol, 
                                   edad_registrada, bmi, systolic, diastolic, care_score, family_risk, exercise_weekly, diet_quality)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(participante_id, dia) DO UPDATE SET
                ...
                edad_registrada=excluded.edad_registrada,
                bmi=excluded.bmi,
                systolic=excluded.systolic,
                diastolic=excluded.diastolic,
                care_score=excluded.care_score,
                family_risk=excluded.family_risk,
                exercise_weekly=excluded.exercise_weekly,
    ...
    def _refresh_participante_combobox(self, select_id=None):
        """Refresca la lista de participantes en el combobox.

        Args:
            select_id: ID del participante que debe seleccionarse inmediatamente (opcional).
        """
        names = [p[1] for p in self.participantes]
        self.participante_cb['values'] = names

        # Si se pide seleccionar un participante específico (después de registro)
        if select_id is not None:
            sel_name = next((p[1] for p in self.participantes if p[0] == select_id), None)
            if sel_name:
                self.participante_cb.set(sel_name)
                self.current_participant_id = select_id
                self.cargar_lecturas_desde_db(select_id)
                if hasattr(self, "tree"):
                    self._refresh_tree()
                    self._set_next_day_and_prefill_inputs()
                return
    ...
    def add_reading(self):
        ...
        if self.current_participant_id:
            print(f"Guardando lectura para participante_id={self.current_participant_id}: {entry}")  # Debug
            self.guardar_lectura_en_db(self.current_participant_id, entry)
            self.lbl_status.config(text=f"Lectura guardada (participante_id={self.current_participant_id}, día={entry['day']})")

            # Refrescar desde BD para asegurar que se ven todos los datos guardados
            self.cargar_lecturas_desde_db(self.current_participant_id)
            self._refresh_tree()
        else:
            print(f"No se guarda lectura (sin participante): {entry}")  # Debug
            self.lbl_status.config(text="No hay participante seleccionado. Seleccione uno antes de agregar lecturas.")
    ...
    def _refresh_tree(self):
        for i in self.tree.get_children():
            self.tree.delete(i)
        print(f"[DEBUG] _refresh_tree: self.readings={self.readings}")
    ...
    def capture_ai_features(self):
        """
        Abre un diálogo para capturar características adicionales para mejorar el entrenamiento de IA.
        Permite al usuario ingresar: edad, IMC, presión arterial, factores de riesgo, etc.
        """
        if not self.current_participant_id:
            messagebox.showwarning("Error", "Seleccione un participante primero.")
            return
        
        # Si no hay lecturas, mostrar advertencia
        if not self.readings:
            messagebox.showwarning("Error", "Agregue al menos una lectura antes de capturar características de IA.")
            return
        
        # Crear ventana de diálogo
        feature_win = tk.Toplevel(self.root)
        feature_win.title("🧠 Características para Entrenamiento de IA")
        feature_win.geometry("500x600")
        feature_win.resizable(True, True)
        
        # Instrucciones
        lbl_intro = ttk.Label(feature_win, text="Ingrese características clínicas adicionales para mejorar el modelo IA:")
        lbl_intro.grid(row=0, column=0, columnspan=2, pady=10, padx=10)
        ...
        # Edad
        ttk.Label(feature_win, text="Edad (años):").grid(row=row, column=0, sticky="w", padx=10, pady=5)
        edad_var = tk.StringVar()
        ttk.Entry(feature_win, textvariable=edad_var, width=15).grid(row=row, column=1, sticky="w", padx=10)
        features['edad'] = edad_var
        row += 1
        
        # IMC
        ttk.Label(feature_win, text="IMC (kg/m²):").grid(row=row, column=0, sticky="w", padx=10, pady=5)
        bmi_var = tk.StringVar()
        ttk.Entry(feature_win, textvariable=bmi_var, width=15).grid(row=row, column=1, sticky="w", padx=10)
        features['bmi'] = bmi_var
        row += 1
        
        # Presión Sistólica
        ttk.Label(feature_win, text="Presión Sistólica (mmHg):").grid(row=row, column=0, sticky="w", padx=10, pady=5)
        syst_var = tk.StringVar()
        ttk.Entry(feature_win, textvariable=syst_var, width=15).grid(row=row, column=1, sticky="w", padx=10)
        features['systolic'] = syst_var
        row += 1
        
        # Presión Diastólica
        ttk.Label(feature_win, text="Presión Diastólica (mmHg):").grid(row=row, column=0, sticky="w", padx=10, pady=5)
        dias_var = tk.StringVar()
        ttk.Entry(feature_win, textvariable=dias_var, width=15).grid(row=row, column=1, sticky="w", padx=10)
        features['diastolic'] = dias_var
        row += 1
        
        # Score de cuidado
        ttk.Label(feature_win, text="Score de cuidado (0-5):").grid(row=row, column=0, sticky="w", padx=10, pady=5)
        care_var = tk.StringVar()
        care_spin = ttk.Spinbox(feature_win, from_=0, to=5, textvariable=care_var, width=13)
        care_spin.set("3")
        care_spin.grid(row=row, column=1, sticky="w", padx=10)
        features['care_score'] = care_var
        row += 1
        
        # Riesgo familiar
        ttk.Label(feature_win, text="Riesgo familiar (0=No, 1=Sí):").grid(row=row, column=0, sticky="w", padx=10, pady=5)
        fam_var = tk.StringVar()
        fam_spin = ttk.Spinbox(feature_win, from_=0, to=1, textvariable=fam_var, width=13)
        fam_spin.set("0")
        fam_spin.grid(row=row, column=1, sticky="w", padx=10)
        features['family_risk'] = fam_var
        row += 1
        
        # Ejercicio semanal
        ttk.Label(feature_win, text="Ejercicio semanal (minutos):").grid(row=row, column=0, sticky="w", padx=10, pady=5)
        ex_var = tk.StringVar()
        ttk.Entry(feature_win, textvariable=ex_var, width=15).grid(row=row, column=1, sticky="w", padx=10)
        features['exercise_weekly'] = ex_var
        row += 1
        
        # Calidad de dieta
        ttk.Label(feature_win, text="Calidad de dieta (1=Pobre, 5=Excelente):").grid(row=row, column=0, sticky="w", padx=10, pady=5)
        diet_var = tk.StringVar()
        diet_spin = ttk.Spinbox(feature_win, from_=1, to=5, textvariable=diet_var, width=13)
        diet_spin.set("3")
        diet_spin.grid(row=row, column=1, sticky="w", padx=10)
        features['diet_quality'] = diet_var
        row += 1
        
        # Botones
        btn_frame = ttk.Frame(feature_win)
        btn_frame.grid(row=row, column=0, columnspan=2, pady=20)
        
        def guardar_features():
            """Guarda las características en las lecturas actuales"""
            try:
                # Validar y convertir valores
                datos = {}
                if edad_var.get().strip():
                    datos['edad_registrada'] = int(edad_var.get())
                if bmi_var.get().strip():
                    datos['bmi'] = float(bmi_var.get())
                if syst_var.get().strip():
                    datos['systolic'] = float(syst_var.get())
                if dias_var.get().strip():
                    datos['diastolic'] = float(dias_var.get())
                if care_var.get().strip():
                    datos['care_score'] = float(care_var.get())
                if fam_var.get().strip():
                    datos['family_risk'] = float(fam_var.get())
                if ex_var.get().strip():
                    datos['exercise_weekly'] = float(ex_var.get())
                if diet_var.get().strip():
                    datos['diet_quality'] = int(diet_var.get())
                
                # Actualizar todas las lecturas con las características
                for lectura in self.readings:
                    lectura.update(datos)
                    # Guardar en BD
                    self.guardar_lectura_en_db(self.current_participant_id, lectura)
                
                messagebox.showinfo("✓ Éxito", f"Características guardadas en {len(self.readings)} lecturas.\nEstos datos ayudarán a mejorar el modelo IA.")
                feature_win.destroy()
                
            except ValueError as e:
                messagebox.showerror("Error", f"Valores inválidos: {e}")
        
        ttk.Button(btn_frame, text="💾 Guardar características", command=guardar_features).pack(side="left", padx=5)
        ttk.Button(btn_frame, text="❌ Cancelar", command=feature_win.destroy).pack(side="left", padx=5)
    ...
    def _build_ia_conclusion(self, stage, prob, timeframe, age, risk_factors, age_stage_text,
                              avg_fast=None, avg_post=None, avg_a1c=None,
                              bmi=None, ldl=None, hdl=None, triglycerides=None,
                              systolic=None, diastolic=None, family_history=0,
                              exercise_weekly=None, diet_quality=None, time_to_diabetes=None,
                              a1c_measured=True):
        """Genera un mensaje final dinámico de IA con análisis clínico más específico."""
        lines = []

        lines.append(f"Edad: {age} años.")
        if bmi is not None:
            lines.append(f"IMC: {bmi:.1f} kg/m².")
        if systolic is not None and diastolic is not None:
            lines.append(f"Presión arterial: {systolic:.0f}/{diastolic:.0f} mmHg.")
        if ldl is not None and hdl is not None and triglycerides is not None:
            lines.append(f"Perfil lipídico: LDL {ldl:.1f}, HDL {hdl:.1f}, triglicéridos {triglycerides:.1f} mg/dL.")

        if avg_fast is not None and avg_post is not None:
            lines.append(f"Glucosa en ayunas: {avg_fast:.1f} mg/dL; glucosa postprandial: {avg_post:.1f} mg/dL.")
        if a1c_measured and avg_a1c is not None:
            lines.append(f"A1C promedio: {avg_a1c:.1f}%.")
        else:
            lines.append("A1C no disponible: la evaluación usa datos de glucosa promedio.")

        if stage == 2:
            lines.append("🔴 Conclusión: Los hallazgos cumplen criterios de diabetes según los umbrales clínicos actuales.")
            if avg_fast is not None and avg_fast >= 126:
                lines.append("Ayuno alto: por encima de 126 mg/dL.")
            if avg_post is not None and avg_post >= 200:
                lines.append("Posprandial alto: por encima de 200 mg/dL.")
            if a1c_measured and avg_a1c is not None and avg_a1c >= 6.5:
                lines.append("A1C en rango de diabetes: 6.5% o más.")
            lines.append("Se recomienda iniciar manejo médico activo y confirmar diagnóstico con pruebas de laboratorio.")
        elif stage == 1:
            lines.append("🟠 Conclusión: Datos compatibles con prediabetes y riesgo elevado.")
            if avg_fast is not None and avg_fast >= 100:
                lines.append("Ayuno en rango de prediabetes: 100-125 mg/dL.")
            if avg_post is not None and avg_post >= 140:
                lines.append("Posprandial en rango de prediabetes: 140-199 mg/dL.")
            if a1c_measured and avg_a1c is not None and avg_a1c >= 5.7:
                lines.append("A1C en rango de prediabetes: 5.7-6.4%.")
            lines.append("Intervención inmediata en estilo de vida y seguimiento médico frecuente son recomendados.")
        else:
            lines.append("🟢 Conclusión: El perfil está dentro de límites cercanos a la normalidad.")
            if avg_fast is not None and avg_fast >= 95:
                lines.append("Ayuno ligeramente elevado, vigilar con controles regulares.")
            if avg_post is not None and avg_post >= 120:
                lines.append("Posprandial levemente alto, mejorar nutrición y actividad física.")
            if a1c_measured and avg_a1c is not None and avg_a1c >= 5.5:
                lines.append("A1C levemente elevado: vigilar y controlar con hábitos saludables.")
            lines.append("Mantener hábitos saludables y reevaluar con pruebas periódicas.")

        if age_stage_text:
            lines.append(age_stage_text)

        if family_history:
            lines.append("Antecedentes familiares de diabetes presentes: mayor vigilancia.")

        if bmi is not None:
            if bmi >= 30:
                lines.append("Obesidad: riesgo adicional importante.")
            elif bmi >= 25:
                lines.append("Sobrepeso: controlar peso para reducir riesgo.")

        if ldl is not None and ldl > 130:
            lines.append("LDL alto: riesgo cardiovascular elevado.")
        if hdl is not None and hdl < 50:
            lines.append("HDL bajo: menor protección cardiovascular.")
        if triglycerides is not None and triglycerides > 150:
            lines.append("Triglicéridos altos: mayor riesgo metabólico.")
        if (systolic is not None and systolic >= 130) or (diastolic is not None and diastolic >= 80):
            lines.append("Presión arterial elevada, controlar presión regularmente.")

        if exercise_weekly is not None and exercise_weekly < 150:
            lines.append("Actividad física insuficiente: menos de 150 min/semana.")
        if diet_quality is not None and diet_quality <= 2:
            lines.append("Dieta pobre: mejorar calidad nutricional con proteínas, fibra y vegetales.")

        if time_to_diabetes is not None:
            lines.append(f"Estimación de progresión: {time_to_diabetes:.1f} años si no se actúa.")

        lines.append(f"Probabilidad IA calculada: {prob*100:.1f}%.")
        lines.append(f"Horizonte de riesgo estimado: {timeframe}.")

        conclusion = "\n".join(lines)
        return conclusion

    def _build_direct_action_plan(self, stage, prob, timeframe, age, a1c_measured,
                                  avg_fast=None, avg_post=None, avg_a1c=None, bmi=None,
                                  ldl=None, hdl=None, triglycerides=None,
                                  systolic=None, diastolic=None,
                                  family_history=0, exercise_weekly=None, diet_quality=None):
        """Construye un resumen directo de riesgo actual y pasos concretos."""
        lines = []
        stage_names = {
            0: "Normal / bajo riesgo",
            1: "Prediabetes / riesgo elevado",
            2: "Diabetes / riesgo activo"
        }
        if stage == 2:
            urgency = "Alta"
        elif stage == 1:
            urgency = "Alta" if prob >= 0.6 else "Moderada"
        else:
            if (avg_fast is not None and avg_fast >= 110) or (avg_post is not None and avg_post >= 140) or (a1c_measured and avg_a1c is not None and avg_a1c >= 5.5) or family_history:
                urgency = "Moderada"
            elif prob >= 0.7:
                urgency = "Moderada"
            else:
        ...
        lines.append(f"Riesgo actual: {stage_names.get(stage, 'Desconocido')} ({prob*100:.1f}%).")
        lines.append(f"URGENCIA: {urgency}.")
        lines.append(f"Horizonte estimado: {timeframe}.")
        if not a1c_measured:
            lines.append("   • A1C pendiente: solicitar análisis de laboratorio para confirmar estado metabólico.")

        if stage == 2:
            lines.append("   • Iniciar manejo médico urgente con endocrinólogo o médico de cabecera.")
            lines.append("   • Confirmar diagnóstico con pruebas de laboratorio adicionales: A1C, perfil lipídico, función renal.")
            lines.append("   • Monitoreo de glucosa diario y control de presión arterial cada visita.")
            if bmi is not None and bmi >= 25:
                lines.append("   • Objetivo: reducir 5-10% de peso corporal si hay sobrepeso u obesidad.")
            if ldl is not None and ldl > 130:
                lines.append("   • Controlar colesterol LDL con dieta, ejercicio y medicación si es necesario.")
            lines.append("   • Iniciar cambios nutricionales claros: menos carbohidratos simples, más fibra y proteínas magras.")
        elif stage == 1:
            lines.append("   • Cambios inmediatos en estilo de vida para impedir progresión a diabetes.")
            lines.append("   • Realizar ejercicio moderado al menos 150 minutos por semana.")
            lines.append("   • Mejorar la calidad de la dieta: evitar azúcares refinados y alimentos procesados.")
            if bmi is not None and bmi >= 25:
                lines.append("   • Perder peso progresivamente si hay sobrepeso u obesidad.")
            if triglycerides is not None and triglycerides > 150:
                lines.append("   • Reducir azúcares y alcohol para bajar triglicéridos.")
            lines.append("   • Revisión de glucosa y A1C en 3-6 meses.")
        else:
            lines.append("   • Mantener hábitos saludables y controles regulares.")
            lines.append("   • Realizar actividad física frecuente y una dieta equilibrada.")
            if bmi is not None and bmi >= 25:
                lines.append("   • Prevenir ganancia de peso y mejorar composición corporal.")
            if family_history:
                lines.append("   • Si hay antecedentes familiares, evaluar nuevamente cada 6-12 meses.")
            else:
                lines.append("   • Reevaluar metabolicamente al menos una vez al año.")

        lines.append("   • Priorizar seguimiento médico y registro de glucosa para tomar decisiones objetivas.")
        return "\n".join(lines)
    ...
    def evaluate_diabetes_risk(self):
        ...
        self._add_question_to_window(eval_win, "A1C promedio", f"{avg_a1c:.1f}%" if a1c_measured else "No disponible", "normal")
        ...
        # Si no hay A1C registrada, usamos un valor neutro estándar para permitir inferencia,
        # pero el reporte indica claramente que el dato real de A1C está ausente.
        model_a1c = avg_a1c if avg_a1c is not None else 5.0
        features = [
            age, avg_fast, avg_post, model_a1c, bmi,
        ...
        prob, stage, time_to_diabetes, risk_factors, recommendations = assess_diabetes_risk_comprehensive(features, model)
        avg_glucose = max(avg_fast, avg_post)
        timeframe = map_probability_to_timeframe(prob, age=age, avg_glucose=avg_glucose, ldl=ldl, hdl=hdl, stage=stage, family_risk_factor=family_risk_factor)
        symptoms = possible_symptoms_by_probability(prob, stage=stage, age=age)
        age_stage_text = describe_age_stage_risk(age, avg_fast, avg_post, avg_a1c)
        ia_summary = self._build_ia_conclusion(
            stage, prob, timeframe, age, risk_factors, age_stage_text,
            avg_fast=avg_fast, avg_post=avg_post, avg_a1c=avg_a1c,
            bmi=bmi, ldl=ldl, hdl=hdl, triglycerides=triglycerides,
            systolic=systolic, diastolic=diastolic, family_history=family_history,
            exercise_weekly=exercise_weekly, diet_quality=diet_quality,
            time_to_diabetes=time_to_diabetes, a1c_measured=a1c_measured
        )
        action_plan = self._build_direct_action_plan(
            stage, prob, timeframe, age, a1c_measured,
            avg_fast=avg_fast, avg_post=avg_post, avg_a1c=avg_a1c, bmi=bmi,
            ldl=ldl, hdl=hdl, triglycerides=triglycerides,
            systolic=systolic, diastolic=diastolic,
            family_history=family_history, exercise_weekly=exercise_weekly,
        ...
        report = (
            ...
            f"🧠 CONCLUSIÓN DE IA\n"
            f"   {ia_summary}\n\n"
            f"📌 RESUMEN DIRECTO DE RIESGO Y PASOS CONCRETOS\n"
            f"{action_plan}\n\n"
        ...
        report += f"   • A1C promedio: {avg_a1c:.1f}%\n" if a1c_measured else "   • A1C promedio: No disponible\n"
    ...
    def _show_report_window(self, report_text):
        ...
        text_widget.tag_configure("urgency_high", foreground="#dc2626", font=("Courier New", 10, "bold"))
        text_widget.tag_configure("urgency_med", foreground="#b45309", font=("Courier New", 10, "bold"))
        text_widget.tag_configure("urgency_low", foreground="#047857", font=("Courier New", 10, "bold"))

        # Resaltar el nivel de urgencia en el reporte
        start_index = "1.0"
        while True:
            idx = text_widget.search("URGENCIA:", start_index, stopindex=tk.END)
            if not idx:
                break
            line_end = text_widget.index(f"{idx} lineend")
            line_text = text_widget.get(idx, line_end)
            if "Alta" in line_text:
                tag = "urgency_high"
            elif "Moderada" in line_text:
                tag = "urgency_med"
            else:
                tag = "urgency_low"
            text_widget.tag_add(tag, idx, line_end)
