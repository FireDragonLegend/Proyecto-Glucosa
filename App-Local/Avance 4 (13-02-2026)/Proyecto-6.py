...
class GlucoseMonitorApp:
    ...
    def reset_week(self):
        if messagebox.askyesno("Confirmar", "¿Desea borrar todas las lecturas de la semana (solo en BD para el participante seleccionado)?"):
    ...
    def _set_next_day_and_prefill_inputs(self):
        ...
        # buscar primer día faltante entre 1 y 7
        next_day = None
        for d in range(1, 8):
            if d not in existing_days:
                ...
                break
        if next_day is None:
            # si ya estaban todos, poner el siguiente lógico (hasta 7)
            next_day = min(7, (existing_days[-1] if existing_days else 0) + 1)
