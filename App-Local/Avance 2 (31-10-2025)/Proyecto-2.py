...
class GlucoseMonitorApp:
    ...
    def assess_with_history(self, new_entry):
        """
        Compara la nueva lectura con las lecturas previas y muestra una alerta
        si hay empeoramiento, valores muy altos o indicios de prediabetes/diabetes.
        """
        # obtener lecturas previas (excluyendo la nueva)
        ...
        # chequeo crítico: valores de diabetes según mediciones actuales o A1C
        if (new_entry['fast'] > 125) or (new_entry['post'] > 199) or (new_entry['a1c'] is not None and new_entry['a1c'] >= 6.5):
            mensajes.append("Valores actuales consistentes con DIABETES (orientativo). Consulte a un profesional de salud de inmediato.")

        # empeoramiento respecto al promedio previo
        # umbrales simples: aumento >10 mg/dL en ayunas o >20 mg/dL en postprandial se considera empeoramiento
        if (new_entry['fast'] - avg_prev_fast) > 10 or (new_entry['post'] - avg_prev_post) > 20:
            mensajes.append("Empeoramiento respecto a lecturas previas (aumento significativo).")

        # indicios de prediabetes si las clasificaciones recientes o promedios están en rango
        ...
        if pred_fast or pred_post or pred_a1c:
            mensajes.append("Indicios de PREDIABETES (orientativo). Considere control y consulta médica.")

        # si no hay mensajes críticos pero la clasificación indica límite alto o prediabetes leve
        if not mensajes:
            # si la nueva lectura está en rangos de límite alto o prediabetes, avisar de seguimiento
            if cf_new in ("Prediabetes", "Diabetes") or cp_new in ("Límite alto", "Prediabetes", "Diabetes") or ca_new in ("Prediabetes", "Diabetes"):
                mensajes.append("La lectura actual requiere seguimiento (rango límite o anómalo).")

        # construir texto resumen con contexto
        if mensajes:
            resumen = (
            ...
            resumen += "\n\n" + "\n".join(mensajes) + "\n\nNota: Esto es orientativo. Consulte a su profesional de salud para diagnóstico."
            # si hay indicios de diabetes usar showerror para destacar; si solo empeoramiento/prediabetes usar showwarning
            if any("DIABETES" in m for m in mensajes):
                messagebox.showerror("Alerta: Valores altos", resumen)
            else:
                messagebox.showwarning("Aviso: Revisar tendencias", resumen)
